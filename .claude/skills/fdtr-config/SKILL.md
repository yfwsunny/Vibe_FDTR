...
| ---------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| Spot size (`--spot-size`)                      | `3.0 um`; use `--spot-x/--spot-y` only for explicit directional spotfit/iterfit workflows                              |
...
| Pipeline (`--pipeline`, `--iterations`)        | `builtin:default` pipeline (fit x spot, y spot-Sr_2-Sz_2,TBC_1), `6` iterations                                        |
| Output path                                    | auto-generated under `tasks/`; no `--output` flag                                                                      |
...
`init-config` does not accept `--output`; use the printed auto-generated path.

**Must** ask user to inspect and confirm the generated config file before proceeding to fit or analysis.

Built-in pipeline references use `builtin:<name>`. File paths for pipeline, offset_dir, phase_dir, etc. use absolute paths as-is; relative paths are resolved relative to the config file's parent directory.
