...
10. **Judge sensitivity by absolute magnitude and curve-shape separability, not
    sign.** A sign change is not an identifiability argument.
...
- Treat `scan-data` grouping as first-pass organization, not final expert
  judgment; assess repeatability, phase integrity, signal quality, xdata
...
- Isolated abnormal points may be removed only with a recorded reason; segment
...
- Spot size is usually `calibrated-fixed`. Treat it as `candidate-fitted` only
  when offset data provide sufficient sensitivity using spotfit, or when strong evidence suggests the nominal spot
  value is unreliable.
...
uncertainty. Do not jump directly to `iterfit`; first evaluate single-strategy
...
6. **Consistency** - are results consistent with literature, calibration, or
   other data types?
7. **Boundary issues** - did any parameter hit its bound?
...
3. **Parameter separability** - are targets sufficiently decoupled from
   nuisance parameters?
4. **Setup robustness** - is the setup robust to reasonable design-variable
   variations?
5. **Offset-region check** - for offset phase, do conclusions hold in the useful
   nonzero-offset region rather than only by full-range averaging?

Acceptance rules:

- If a parameter hits its bound, the fit is constrained by the bound, not by the
  data. Investigate before trusting the result.
- A partial-range fit is acceptable only with explanation and supporting
  evidence.
...
- Before presenting a tool-backed conclusion, check that the config path is
  correct, the command succeeded, and the cited output file exists.
