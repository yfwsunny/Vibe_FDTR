...
- Built-in material files live in `src/fdtr/materials/data/`; use `uv run fdtr list-materials` and `uv run fdtr show-material <name> -T <K>` before searching files. In configs, use the material stem such as `Gold_film`, not `Gold_film.txt`.
3. **Core function**
...
- `iterfit`: pipeline framework that repeats multiple fitting steps (e.g. offset then freq) in a loop until converged. To customize the pipeline, load `.claude/skills/fdtr-iterpipeline/SKILL.md`.

Do not use `iterfit` as a sensitivity strategy.

`iterfit` is only for repeated alternating fitting loops or explicit pipeline-based coupled estimation. Do not choose `iterfit` merely because the overall task has multiple steps, multiple datasets, or both offset and frequency data.

Do not orchestrate separate freqfit/offsetfit runs when the user describes an iterative workflow.
