...
- **Post-fit mode**: use a fit-result JSON when available. Do not reconstruct fitted state from report text or console output.
...
| `--target-params` (repeat one per target.) | no                     | Parameters whose uncertainty is evaluated. Use `Param_INDEX` naming. Repeat for multiple targets.                                                                                |
