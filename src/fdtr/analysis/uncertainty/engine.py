...
def _iterfit_spot_for_key(config: FitConfig, spot_key: str | None) -> float:
    """Return the scalar spot that an iterfit pipeline step should use."""
    if spot_key == "spot_x":
        return float(config.spot_x or config.spot_size or config.spot_y or 3.0)
    if spot_key == "spot_y":
        return float(config.spot_y or config.spot_x or config.spot_size or 3.0)
    if spot_key in ("spot_size", "spot_avg"):
        directional = average_directional_spots(config.spot_x, config.spot_y)
        if directional is not None:
            return directional
    return _iterfit_scalar_spot(config)


def _with_step_spot(config: FitConfig, spot_size_um: float, func):
    """Run ``func`` while exposing a step-specific scalar spot to helpers."""
    saved_spot_size = config.spot_size
    config.spot_size = float(spot_size_um)
    try:
        return func()
    finally:
        config.spot_size = saved_spot_size
...
def _valid_spot_known_name(spot_key: str | None) -> str | None:
    if spot_key in {"spot_size", "spot_x", "spot_y"}:
        return spot_key
    if spot_key == "spot_avg":
        return "spot_size"
    return None
...
def run_iterfit_uncertainty(config) -> dict:
    ...
    user_spot_names = [name for name in user_known if name in _SPOT_NAMES]
    ignore_user_spot_known = bool(fitted_spot_names)

    if len(user_spot_names) > 1:
        warnings.warn(
            "Multiple spot known_params were provided; each iterfit step uses "
            "only the spot parameter selected by its spot_key.",
            UserWarning,
            stacklevel=2,
        )
    if ignore_user_spot_known and user_spot_names:
        warnings.warn(
            "User spot known_params were ignored because spot is fitted by "
            f"iterfit target(s): {', '.join(sorted(fitted_spot_names))}.",
            UserWarning,
            stacklevel=2,
        )
    ...
    for _iteration in range(max_iterations):
        ...
        for step in pipeline.steps:
            ...
            if exclude_names & _SPOT_NAMES:
                step_spot_key = next((name for name in step.target_names if name in _SPOT_NAMES), None)
                spot_allowed = None
                exclude_names |= _SPOT_NAMES
            else:
                step_spot_key = getattr(step, "spot_key", "spot_size")
                spot_allowed = _valid_spot_known_name(step_spot_key)

            spot_exclude = _SPOT_NAMES - {spot_allowed} if spot_allowed else _SPOT_NAMES
            step_spot_size = _iterfit_spot_for_key(fit_config, step_spot_key)

            # Build known params with spot-aware exclusion
            full_exclude = _expand_isotropic_underlying(exclude_names | spot_exclude)
            known_names: list[str] = []
            known_rel: dict[str, float] = {}

            # Priority 1: forward-propagated uncertainties from previous steps.
            for prev_param, rel_sigma in resolved_sigma.items():
                if prev_param not in full_exclude and prev_param not in known_rel:
                    known_names.append(prev_param)
                    known_rel[prev_param] = rel_sigma

            # Priority 2: user-provided known uncertainties.
            for name, rel_sigma in user_known.items():
                if name in _SPOT_NAMES and ignore_user_spot_known:
                    continue
                if name not in full_exclude and name not in known_rel:
                    known_names.append(name)
                    known_rel[name] = rel_sigma
            ...
            if (
                spot_allowed
                and spot_allowed not in known_rel
                and spot_allowed not in full_exclude
            ):
                known_names.append(spot_allowed)
                known_rel[spot_allowed] = 0.05
            ...
            if analysis_kind == "fwhm":
                ...
                Ju, Jc = _with_step_spot(
                    fit_config,
                    step_spot_size,
                    lambda: (
                        compute_fwhm_jacobian(fit_config, target_params, x_data, freq_hz),
                        compute_fwhm_jacobian(fit_config, known_resolved_params, x_data, freq_hz),
                    ),
                )
            else:
                ...
                try:
                    ...
                    Ju, Jc = _with_step_spot(
                        ...
                        step_spot_size,
                        lambda: (
                            compute_jacobian(fit_config, target_params, x_data, analysis_kind),
                            compute_jacobian(fit_config, known_resolved_params, x_data, analysis_kind),
                        ),
                    )
                finally:
            ...
            for kn in known_names:
                val = get_param_value(stack, kn, spot_size_um=step_spot_size)
                rel = known_rel[kn]
            ...
            for i, param in enumerate(target_params):
                p_val = get_param_value(stack, param.raw_name, spot_size_um=step_spot_size)
