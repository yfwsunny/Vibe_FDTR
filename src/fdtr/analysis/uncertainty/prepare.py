...
def _merged_known_param_config(
    ...
    user_spot_names = [name for name in overrides if name in _SPOT_NAMES]
    ...
    for param in target_params:
        ...
        elif param.kind == "spot_size":
            target_name_set |= _SPOT_NAMES

    invalid = sorted(
        name for name in overrides
        if name not in valid_names and name not in _SPOT_NAMES
    )
    if invalid:
        raise ValueError("Invalid known_params entries: " + ", ".join(invalid))

    spot_targets = target_name_set & _SPOT_NAMES
    if user_spot_names and spot_targets:
        warnings.warn(
            "User spot known_params were ignored because spot is a target "
            "parameter in this uncertainty calculation.",
            UserWarning,
            stacklevel=2,
        )
    elif len(user_spot_names) > 1:
        warnings.warn(
            "Multiple spot known_params were provided; scalar uncertainty "
            "uses spot_size and ignores directional spot_x/spot_y entries.",
            UserWarning,
            stacklevel=2,
        )
    elif any(name in {"spot_x", "spot_y"} for name in user_spot_names):
        warnings.warn(
            "Directional spot known_params were ignored because this "
            "uncertainty calculation uses scalar spot_size.",
            UserWarning,
            stacklevel=2,
        )
    ...
    for name, value in overrides.items():
        if name in _SPOT_NAMES:
            if spot_targets or name != "spot_size":
                continue
