...
class OffsetFitter:
    ...
    def fit(
        ...
        preprocessed = self._preprocess(offset_data, signal_data)
    ...
    def _preprocess(
        self, offset_data: np.ndarray, signal_data: np.ndarray,
    ) -> dict:
        ...
        peak_idx = int(np.argmax(signal_data))
        peak_offset = float(offset_data[peak_idx])
        signal_peak = float(signal_data[peak_idx])
        centered_offset_full = offset_data - peak_offset
