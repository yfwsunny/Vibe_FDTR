...
def _plot_and_save_iter_results(
    ...
    if "offset_x_phase" in datasets:
        ...
        sr_off = FitResult(
            fitted_values={offset_key: substrate.Sr},
            residual=next(
                (residuals.get(step.name, 0.0) for step in pipeline.steps if step.fitter == "offset"),
                0.0,
            ),
            success=True, nfev=0,
            fitted_stack=final_stack,
        ...
        curve = calculate_offset_fit_curve(
            phase_sep_sub, phase_offset_sub, config.freq_offset, config, sr_off, max_points=_MAX_PLOT_PTS
        )
        fig = plot_offset_fit(phase_sep_sub, phase_offset_sub, config.freq_offset,
    ...
    if "freq_phase" in datasets:
        ...
        sr_freq = FitResult(
            fitted_values={substrate_key: sz_val, tbc_key: tbc_val},
            residual=next(
                (residuals.get(step.name, 0.0) for step in pipeline.steps if step.fitter == "freq"),
                0.0,
            ),
            success=True, nfev=0,
            fitted_stack=final_stack,
