...
def _load_spot_scans(config, data_file_x, data_file_y):
    if data_file_x is not None:
        ...
        scan_x = load_offset_scan(str(data_file_x))
        scan_y = None
        if data_file_y is not None:
            ...
            scan_y = load_offset_scan(str(data_file_y))
...
def run_spotfit(config: FitConfig, args=None) -> Optional[FitResult]:
    ...
    # --- Save X PNG + CSV --------------------------------------------------
    curve_x = calculate_spot_fit_curve(sep_um, amp_data_x, stack, spot_x, actual_freq)
    fig_x = plot_spot_fit(
        sep_um=sep_um, amp_data=amp_data_x, stack=stack,
    ...
    if scan_y is not None and result_y is not None and sep_um_y is not None and amp_data_y is not None:
        ...
        curve_y = calculate_spot_fit_curve(sep_um_y, amp_data_y, stack, spot_y, y_freq)
        fig_y = plot_spot_fit(
            sep_um=sep_um_y, amp_data=amp_data_y, stack=stack,
