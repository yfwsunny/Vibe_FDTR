...
def resolve_guess(config: FitConfig, target: TargetSpec) -> float:
    ...
    if name in ("spot_x", "spot_y"):
        axis_value = getattr(config, name)
        if axis_value is not None:
            return axis_value
        if config.spot_size is None:
            raise ValueError(
                f"Cannot auto-resolve guess for '{name}': "
                f"config.{name} and config.spot_size are not set."
            )
        return config.spot_size
    ...
    if config.fit_spot_x is not None:
        all_targets.append(
            TargetSpec(
                ...
                guess=config.spot_x if config.spot_x is not None else config.spot_size,
            )
        )
    if config.fit_spot_y is not None:
        all_targets.append(
            TargetSpec(
                ...
                guess=config.spot_y if config.spot_y is not None else config.spot_size,
            )
        )
