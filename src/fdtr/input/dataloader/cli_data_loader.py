...
def _resolve_data_source(
    ...
    """Normalize a file-or-directory source into a search directory and filter."""
    if data_path.is_file():
        if explicit_files is None:
            return data_path.parent, [data_path.name]
        return data_path.parent, explicit_files
    return data_path, explicit_files


def _build_overlap_grid(coord_arrays: list[np.ndarray]) -> np.ndarray:
    """Build a sorted averaging grid restricted to the shared coordinate range."""
    finite_arrays = [np.asarray(arr, dtype=np.float64)[np.isfinite(arr)] for arr in coord_arrays]
    if not finite_arrays or any(arr.size == 0 for arr in finite_arrays):
        raise ValueError("Cannot average scans with empty coordinate arrays.")

    lo = max(float(arr.min()) for arr in finite_arrays)
    hi = min(float(arr.max()) for arr in finite_arrays)
    if lo > hi:
        raise ValueError("Cannot average scans without overlapping coordinate ranges.")

    overlap_parts = [arr[(arr >= lo) & (arr <= hi)] for arr in finite_arrays]
    overlap = np.unique(np.concatenate([arr for arr in overlap_parts if arr.size > 0]))
...
def _load_offset_scan_data(
    ...
    Args:
        data_dir: Directory or single file to search/load for offset scan data.
    ...
    """
    result: dict = {}
    data_dir, explicit_files = _resolve_data_source(data_dir, explicit_files)
    ...
    for direction, selector, explicit in [
        ...
        if average and len(files) > 1:
            ...
            common_sep = _build_overlap_grid([s.offset for s in scans])
...
def _load_freq_sweep_data(
    ...
    Args:
        data_dir: Directory or single file to search/load for freq-sweep data.
        average: Whether to average multiple files.
    ...
    if average and len(files) > 1:
        ...
        common_freq = _build_overlap_grid([s.frequency for s in sweeps])
