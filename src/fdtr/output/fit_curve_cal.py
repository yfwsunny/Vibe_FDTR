...
def calculate_offset_fit_curve(
    ...
    peak_idx = int(np.argmax(signal_out))
    sep_centered = sep_out - sep_out[peak_idx]

    if result.fitted_stack is None:
        model = np.full_like(signal_out, np.nan, dtype=float)
        data = signal_out - signal_out[peak_idx] if signal == SignalType.PHASE else signal_out
    ...
    if signal == SignalType.PHASE:
        ...
        data = signal_out - signal_out[peak_idx]
    ...
    amp_data_centered = signal_out - signal_out[peak_idx]
